...
export default function ProductosListaClient({ categorias = [] }) {
  const { user } = useUser();
  ...
  const productosOrdenados = (() => {
    if (orden === "reciente") return todosProductos.slice().sort((a, b) => b.id - a.id);
    if (orden === "antiguo") return todosProductos.slice().sort((a, b) => a.id - b.id);
