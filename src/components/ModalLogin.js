...
export default function ModalLogin({
  ...
  return (
    ...
    >
      <div
        className={`relative w-[90%] max-w-md rounded-3xl bg-white p-7 shadow-[0_20px_60px_rgba(0,0,0,.35)] transition-all duration-300 transform ${
          isOpen
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-red-500 hover:bg-red-100/80 rounded-full p-1 transition hover:scale-110 cursor-pointer"
        >
          <X size={26} />
        </button>
        ...
        {/* Mensaje de éxito verde dentro del modal */}
        {success && (
          <div className="mb-4">
        ...
        <div className="mb-3">
          ...
          {errorUser && (
            <p
              key={errorUserKey}
              className="bg-red-100 text-red-700 text-xs text-center rounded-lg px-2 py-1 mt-2"
            >
              {errorUser}
            </p>
          )}
