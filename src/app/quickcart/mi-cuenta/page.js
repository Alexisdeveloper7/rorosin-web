...
export default function MiCuentaPage() {
      ...
      try {
        ...
        if (data.ok) {
          ...
          if (data.usuario.estado) {
            const estadoObj = estadosMX.find(
              (e) => e.name === data.usuario.estado
            );

            setCiudadesEstado(
              estadoObj
                ? City.getCitiesOfState("MX", estadoObj.isoCode)
                : []
            );
          }
        }
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (
        editEstado &&
        estadoRef.current &&
        !estadoRef.current.contains(e.target)
      ) {
        setEditEstado(false);
      }
      ...
      if (
        editCiudad &&
        ciudadRef.current &&
        !ciudadRef.current.contains(e.target)
      ) {
        setEditCiudad(false);
      }
    };
  ...
  useEffect(() => {
    if (editCiudad && inputCiudadRef.current) {
      ...
      <div className="w-full max-w-xl sm:max-w-2xl bg-white shadow-xl rounded-xl p-5 space-y-5">
        ...
        {user && datos && !loading && !cargandoDatos && (
          <div className="space-y-5">
                ...
                editUsuario ? (
                  <div className="flex flex-wrap gap-2 items-center w-full">
                    ...
                    <button
                      ...
                      className="px-5 py-1 text-sm font-medium rounded-md bg-green-600 text-white hover:bg-green-700 flex-shrink-0"
                    >
                      Guardar
                    </button>
                    ...
                    <button
                      ...
                      className="px-5 py-1 text-sm font-medium rounded-md bg-gray-400 text-white hover:bg-gray-500 flex-shrink-0"
                    >
                      Cancelar
                    </button>
                  </div>
            ...
            <Fila
              label="País"
              value={<span className="ml-2">{datos.pais}</span>}
            />
              ...
              value={
                <div className="relative w-full ml-2">
                  ...
                  {editEstado && (
                    <div
                      ref={estadoRef}
                      className="absolute top-full left-0 w-full z-50 bg-white border rounded-lg shadow-lg max-h-56 overflow-y-auto mt-1"
                    >
                      <div className="sticky top-0 bg-white px-3 py-2 font-semibold border-b text-sm">
                      ...
                      {estadosMX.map((e) => (
                        <div
                          ...
                          onClick={() => seleccionarEstado(e.name)}
                        >
                          {e.name}
                        </div>
                      ))}
            ...
            <Fila
                        ...
                        {ciudadesFiltradas.length > 0
                          ? ciudadesFiltradas.map((c) => (
                              <div
                                key={c.name}
                                className="px-3 py-1 hover:bg-gray-200 cursor-pointer text-sm"
                                onClick={() => seleccionarCiudad(c.name)}
                              >
                                {c.name}
                              </div>
                            ))
              ...
              onClick={() => {
                if (datos.estado) setEditCiudad(true);
              }}
...
function Fila({ label, value, boton, onClick, color, disabled }) {
  ...
  return (
    <div className="flex justify-between items-center bg-gray-50 px-3 py-3 rounded-md relative">
      <span className="text-sm font-semibold text-gray-600 w-36">
        {label}
      </span>
